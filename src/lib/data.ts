export const COMPANY_INFO = {
  name: "Boomerang Estimating",
  tagline: "Estimates That Always Hit the Mark",
  phone: "1300 ESTIMATE",
  email: "info@boomerangestimating.com.au",
  address: "Sydney, NSW, Australia",
  hours: "Monday-Friday, 9:00 AM – 6:00 PM PKT",
  whatsapp: "https://wa.me/1234567890",
  socials: {
    linkedin: "https://www.linkedin.com/company/boomerang-estimating",
    facebook: "#",
    instagram: "#"
  }
};

export const SERVICES = [
  {
    id: "residential-takeoffs",
    title: "Residential Takeoffs",
    slug: "residential-takeoffs",
    shortDescription: "Accurate quantity takeoffs for custom homes, renovations, and multi-unit developments.",
    description: "We provide comprehensive residential estimating services tailored to Australian builders. From single custom homes to large-scale residential developments, our detailed takeoffs ensure you have an accurate understanding of all material and labor costs.",
    price: "from AUD $150",
    turnaround: "24-48h",
    deliverables: ["Detailed Material List", "Labor Cost Estimation", "Markup Adjustments", "PDF & Excel Formats"],
    icon: "Home"
  },
  {
    id: "commercial-estimating",
    title: "Commercial Estimating",
    slug: "commercial-estimating",
    shortDescription: "Detailed cost estimation for retail, office, warehouse, and industrial projects.",
    description: "Our commercial estimating services are built for the fast-paced Australian commercial sector. We deliver precise, structured estimates that help you submit competitive tenders and win more commercial projects without sacrificing margin.",
    price: "from AUD $400",
    turnaround: "48-72h",
    deliverables: ["Full Trade Breakdown", "BOQ Preparation", "Risk Assessment", "Value Engineering Options"],
    icon: "Building2"
  },
  {
    id: "civil-structural-estimation",
    title: "Civil & Structural",
    slug: "civil-structural-estimation",
    shortDescription: "Specialized estimation for earthworks, concrete, steel, and infrastructure.",
    description: "Handling complex civil and structural projects requires deep expertise. Our specialized team delivers highly accurate structural and civil estimates compliant with Australian standards, covering everything from bulk earthworks to detailed steel fabrication.",
    price: "from AUD $600",
    turnaround: "72-96h",
    deliverables: ["Earthworks Quantification", "Steel Tonnage Reports", "Concrete Volume Analysis", "Plant & Equipment Costs"],
    icon: "Ruler"
  }
];

export const PORTFOLIO = [
  {
    id: "1",
    title: "Luxury Coastal Residence",
    slug: "luxury-coastal-residence",
    category: "Residential",
    location: "Sydney, NSW",
    description: "Complete quantity takeoff and cost estimation for a high-end 5-bedroom coastal home.",
    fullDescription: "Provided a comprehensive residential estimate including detailed material lists, labor costs, and contingencies for a high-end architectural home in Sydney. The project featured complex curved concrete walls and extensive structural steel.",
    delivered: ["Architectural Takeoff", "Structural Takeoff", "BOQ", "Tender Review"]
  },
  {
    id: "2",
    title: "Metro Office Complex",
    slug: "metro-office-complex",
    category: "Commercial",
    location: "Melbourne, VIC",
    description: "Detailed BOQ and tender pricing for a 6-story commercial office building.",
    fullDescription: "Delivered a full Bill of Quantities (BOQ) for a mid-rise commercial development. Our estimate helped the main contractor secure the project by identifying key value-engineering opportunities in the facade system.",
    delivered: ["Full BOQ", "Subcontractor Package Preparation", "Value Engineering", "Cost Planning"]
  },
  {
    id: "3",
    title: "Highway Upgrade Project",
    slug: "highway-upgrade-project",
    category: "Civil & Structural",
    location: "Brisbane, QLD",
    description: "Earthworks and structural concrete estimation for a major intersection upgrade.",
    fullDescription: "A highly complex civil project requiring precision earthworks quantification and structural concrete detailing for new bridge abutments. Delivered exactly to the principal contractor's formatting requirements.",
    delivered: ["Earthworks Takeoff", "Concrete Volumetrics", "Reinforcement Tonnage", "Traffic Management Allowances"]
  },
  {
    id: "4",
    title: "Suburban Townhouse Development",
    slug: "suburban-townhouse-development",
    category: "Residential",
    location: "Perth, WA",
    description: "Full cost planning for a 12-unit residential townhouse development.",
    fullDescription: "Provided preliminary cost planning and detailed pre-tender estimates for a multi-unit development, giving the developer clear visibility on project feasibility.",
    delivered: ["Feasibility Estimate", "Detailed Takeoff", "Trade Breakdowns"]
  },
  {
    id: "5",
    title: "Regional Shopping Center",
    slug: "regional-shopping-center",
    category: "Commercial",
    location: "Adelaide, WA",
    description: "Comprehensive estimating for a new retail precinct.",
    fullDescription: "Handled the full estimation package for a regional retail center, encompassing base build and integrated fit-outs for major anchor tenants.",
    delivered: ["Base Build Estimate", "Fit-out Allowances", "Services Coordination Estimation"]
  },
  {
    id: "6",
    title: "Industrial Warehouse Facility",
    slug: "industrial-warehouse-facility",
    category: "Civil & Structural",
    location: "Sydney, NSW",
    description: "Structural steel and concrete estimation for a 5000sqm warehouse.",
    fullDescription: "Detailed estimation of structural components for a large-scale logistics facility, ensuring highly accurate steel tonnage and concrete volume reports.",
    delivered: ["Structural Steel Takeoff", "Slab & Footing Concrete", "Precast Panel Quantification"]
  }
];

export const TESTIMONIALS = [
  {
    id: 1,
    name: "James Wilson",
    company: "BuildRight Pty Ltd",
    rating: 5,
    text: "Boomerang Estimating has completely transformed how we handle tenders. Their accuracy is unmatched, and hitting the 48-hour turnaround consistently has helped us win 3 major residential projects this quarter."
  },
  {
    id: 2,
    name: "Sarah Chen",
    company: "Apex Constructions",
    rating: 5,
    text: "We needed a reliable partner for commercial BOQs. Noman and his team deliver estimates that are structured perfectly for the Australian market. Highly recommended."
  },
  {
    id: 3,
    name: "Michael O'Connor",
    company: "O'Connor Civil",
    rating: 5,
    text: "The detail in their civil and structural takeoffs is exceptional. They caught discrepancies in the engineering plans that saved us thousands before we even broke ground."
  },
  {
    id: 4,
    name: "David Rossi",
    company: "Coastal Homes",
    rating: 5,
    text: "Affordable, fast, and incredibly precise. Boomerang is now an integrated part of our pre-construction process."
  },
  {
    id: 5,
    name: "Emma Thompson",
    company: "ET Developments",
    rating: 5,
    text: "Communication is excellent. Despite being remote, they feel like an in-house team. The pricing is very competitive for the level of detail provided."
  },
  {
    id: 6,
    name: "Liam Davies",
    company: "Prime Commercial",
    rating: 5,
    text: "Outstanding service. The risk assessments included in their commercial estimates give us total confidence when submitting our final tender figures."
  }
];

export const FAQS = [
  {
    question: "How long does an estimate take?",
    answer: "Our standard turnaround times depend on the project type: 24-48 hours for Residential, 48-72 hours for Commercial, and 72-96 hours for Civil & Structural projects. We'll always confirm the exact timeframe when you submit your plans."
  },
  {
    question: "What file formats do you accept?",
    answer: "We prefer plans in PDF or DWG format. We also accept ZIP files for large plan sets, as well as XLSX, JPG, and PNG formats for supporting documentation."
  },
  {
    question: "Do you work to Australian standards?",
    answer: "Yes, absolutely. Our estimating team is highly experienced in the Australian construction industry and strictly adheres to local building codes, terminology, and standard measurement methods."
  },
  {
    question: "How do you deliver estimates?",
    answer: "We provide comprehensive reporting. You'll receive a detailed PDF report summarizing the estimate, along with fully unlocked Excel files (like a BOQ or Trade Breakdown) so you can adjust margins or rates as needed."
  },
  {
    question: "What are your payment terms?",
    answer: "For new clients, we typically request a 50% upfront deposit before commencing work, with the remainder due upon delivery. For ongoing clients, we offer flexible invoicing terms."
  },
  {
    question: "Can you handle urgent projects?",
    answer: "Yes. If you have an urgent tender deadline, please let us know immediately. We can often accommodate rush jobs for a small priority fee."
  },
  {
    question: "What types of projects do you estimate?",
    answer: "We cover the full spectrum: single custom homes, multi-unit developments, retail fit-outs, office buildings, warehouses, earthworks, infrastructure, and heavy structural projects."
  },
  {
    question: "Do you offer revisions?",
    answer: "Yes. We include minor revisions (such as adjusting a specific rate or accommodating a small plan addendum) free of charge within 7 days of delivering the estimate. Major architectural changes will be quoted as a variation."
  }
];